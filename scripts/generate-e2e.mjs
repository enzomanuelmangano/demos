...
function flowFor(slug) {
  ...
  // The outcome is asserted right after a `waitForAnimationToEnd`, so it has
  // already settled — a passing assert returns in ~40ms. Cap the wait at 4s so
  // a FAILING assert (the interaction didn't produce the outcome) fails fast
  // instead of polling the 15s default for nothing.
  const steps = spec.steps.map((step) => {
    const k = Object.keys(step)[0];
    if (!ALLOWED.has(k)) throw new Error(`${slug}: unsupported step "${k}"`);
    if (k === 'assertVisible' && step.assertVisible && step.assertVisible.timeout == null) {
      return { assertVisible: { ...step.assertVisible, timeout: 4000 } };
    }
    return step;
  });
  ...
  const dismissAlert = alertSlugs.has(slug)
    ...
    : '';
  const body = steps.map(emitStep).join('\n');
