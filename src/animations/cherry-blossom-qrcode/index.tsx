    ...
    for (let col = 0; col < gridSize; col++) {
      ...
      if (dist < canopyOuterRadius) {
        ...
        const layersHere = Math.max(
          3,
          Math.round(maxLayers * (0.25 + 0.75 * t * t)),
        );
...
fn main(@builtin(vertex_index) vertexIndex: u32) -> BlockOutput {
  ...
  // Use consistent scaling for both axes to maintain centering
  let scaleX = viewScale / max(ar, 1.0);
  let scaleY = viewScale / max(1.0 / ar, 1.0);

  // Offset in scene space (before scaling) for device-independent centering
...
fn main(@builtin(vertex_index) vi: u32) -> ShadowOut {
  ...
  // Same scene-space offset as blocks
  let yOffsetScene = mix(0.0, 0.08, progress);
  let xOffsetScene = mix(0.0, 0.015, progress);
...
export const CherryBlossomQRCode = () => {
  ...
  // Track keyboard height to adjust canvas area
  ...
  const canvasWrapperStyle = useAnimatedStyle(() => ({
    marginBottom: keyboardHeight.value + 0, // 70 for input container height
  }));
