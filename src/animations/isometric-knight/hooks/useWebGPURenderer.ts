...
export function useWebGPURenderer(
  ...
  const initWebGPU = useCallback(async () => {
    ...
    const context = canvasRef.current.getContext('webgpu') as RNWebGPUContext | null;
    if (!context) {
  ...
  const render = useCallback(() => {
    ...
    const { device, context, pipeline, bindGroup, uniformBuffer, depthTexture } =
      resourcesRef.current;
    ...
    if (isSpringSettled(animState.progress, animState.progressVelocity, viewTarget)) {
    ...
    if (isSpringSettled(animState.dataProgress, animState.dataVelocity, dataTarget)) {
    ...
    const renderPass = commandEncoder.beginRenderPass({
      colorAttachments: [
        {
          view: textureView,
          clearValue: { r: SURFACE_RGB.r, g: SURFACE_RGB.g, b: SURFACE_RGB.b, a: 1 },
          loadOp: 'clear',
          storeOp: 'store',
        },
      ],
